...
def recalculate_temp(regulationType, t_desired):
    ...
    # Rebuild neighbor cache if layout changed
    if _layout_dirty:
        _rebuild_neighbor_cache()
    
    # Compute heater signal ONCE using sensor history (not full temp grids!)
    heater_signal = 0.0
    _group_signals = [0.0] * c.SENSORS_GROUPS_COUNT
    for group_idx in range(c.SENSORS_GROUPS_COUNT):
        if sensors[group_idx] and sensors_average_history:
            # Извлекаем историю ТОЛЬКО этой группы
            sensor_history = [
                entry[group_idx] 
            ...
            if sensor_history:
                if regulationType == 'r':
                    ...
                    signal = 0.0
                _group_signals[group_idx] = max(signal, 0.0) 
    # if sensors and sensors[0] and sensors_average_history:
    #     sensor_history = [entry[0] for entry in sensors_average_history if len(entry) > 0]
    #     if sensor_history:
    #         if regulationType == 'r':
    #             heater_signal = ReleRegulation(100, t_desired, sensor_history)
    #         elif regulationType == 'p':
    #             heater_signal = PRegulation(1.5, t_desired, sensor_history)
    #         elif regulationType == 'pi':
    #             heater_signal = PIRegulation(1.5, 0.5, t_desired, sensor_history)
    #         elif regulationType == 'pid':
    #             heater_signal = PIDRegulation(1.5, 0.5, 0.1, t_desired, sensor_history)
    #         heater_signal = max(heater_signal, 0.0)
    
    # Create new temperature frame (Jacobi method - read from old, write to new)
    ...
    for i in range(c.ROOM_SIZE_Y):
        for j in range(c.ROOM_SIZE_X):
            ...
            # Skip calculation for heaters and outdoor cells (they don't change temp)
            if colors[i][j] in (c.HEATER_COLOR, c.OUTDOOR_COLOR):
                new_temperatures[i][j] = current_temp
                continue
            
            # Process neighbors using precomputed cache
            for ni, nj, k_base in _neighbor_cache[i][j]:
                neighbor_temp = old_temps[ni][nj]
                dt_local = neighbor_temp - current_temp
                
                # Special handling for heater neighbors
                #if colors[ni][nj] == c.HEATER_COLOR:
                #    dtemp += K_signal * heater_signal * dt_local
                #else:
                #    dtemp += k_base * dt_local
                if colors[ni][nj] == c.HEATER_COLOR:
                    heater_group = _heater_group_map[ni][nj]
                    if heater_group >= 0:  # Нашли группу нагревателя
                        signal = _group_signals[heater_group]
                        dtemp += K_signal * signal * dt_local
                    else:
                        # Резервный вариант (не должно происходить)
                        dtemp += K_signal * _group_signals[0] * dt_local
                elif colors[ni][nj] == c.AC_COLOR:
                    ac_group = _ac_group_map[ni][nj]
                    if ac_group >= 0:
                        ac_signal = _group_signals[ac_group] 
                        if ac_signal >= 0:
                            power = ac_signal / 100.0 
                            
                            heat_flux = power * (45.0 - current_temp) * 0.9
                            dtemp += K_signal * heat_flux
                        elif ac_signal < 0:
                            power = -ac_signal / 100.0
                            cool_flux = power * (current_temp - 5.0) * 0.9
                            dtemp -= K_signal * cool_flux
    ...
    temperatures[0] = new_temperatures  # Maintain temperatures[-1] compatibility
    
    # Update colors for dynamic cells (floor-like)
    for i in range(c.ROOM_SIZE_Y):
        for j in range(c.ROOM_SIZE_X):
