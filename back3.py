...
def recalculate_temp(regulationType, t_desired):
    ...
    # Вычисляем базовый сигнал для каждой группы (как раньше)
    ...
    # === КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: устанавливаем рабочую температуру КОНДИЦИОНЕРА в зависимости от знака сигнала ===
    # Делаем это ДО теплового расчёта, чтобы соседи "видели" правильную температуру источника
    for ac in air_conditioners:
        group_idx = ac.sensor_group
        if 0 <= group_idx < len(base_signals):
            signal = base_signals[group_idx]
            # Положительный сигнал → режим НАГРЕВА (45°C)
            # Отрицательный сигнал → режим ОХЛАЖДЕНИЯ (5°C)
            ac_target_temp = 45.0 if signal > 0 else 5.0
            _current_temperatures[ac.Y][ac.X] = ac_target_temp
    
    # Тепловой расчёт (как раньше, но теперь кондиционер имеет правильную температуру)
    ...
    for i in range(c.ROOM_SIZE_Y):
        for j in range(c.ROOM_SIZE_X):
            ...
            # Источники температуры не пересчитываются (сохраняют фиксированное значение)
            ...
            for ni, nj, k_base in _neighbor_cache[i][j]:
                ...
                if colors[ni][nj] == c.HEATER_COLOR:
                    ...
                    if 0 <= heater_group < len(base_signals):
                        signal = base_signals[heater_group]
                        power = max(signal, 0.0)  # нагреватель только греет
                        dtemp += K_signal * power * dt_local
                elif colors[ni][nj] == c.AC_COLOR:
                    ac_group = _ac_group_map[ni][nj]
                    if 0 <= ac_group < len(base_signals):
                        signal = base_signals[ac_group]
                        power = abs(signal)  # кондиционер работает с полной мощностью в любом режиме
                        dtemp += K_signal * power * dt_local
                else:
    ...
    # Защита: принудительно сохраняем фиксированные температуры источников
    for heater in heaters:
    ...
    # Обновляем цвета для динамических ячеек (пол)
    for i in range(c.ROOM_SIZE_Y):
        for j in range(c.ROOM_SIZE_X):
