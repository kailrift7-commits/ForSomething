...
# Global state
K_window, K_door, K_wall, K_air = c.K_window, c.K_door, c.K_wall, c.K_air # 0.02, 0.01, 0.008, 0.1
...
def recalculate_temp(regulationType, t_desired):
        ...
        if sensors[group_idx] and sensors_average_history:
            ...
            if sensor_history:
                if regulationType == 'r':
                    ...
                    signal = PIRegulation(12, 0.5, t_desired, sensor_history)
    ...
    # === УСТАНОВКА ТЕМПЕРАТУРЫ КОНДИЦИОНЕРА ДО РАСЧЁТА (тепловой насос) ===
    for ac in air_conditioners:
        group_idx = ac.sensor_group
        if 0 <= group_idx < len(base_signals):
            signal = base_signals[group_idx]
            # Положительный сигнал → НАГРЕВ (45°C), Отрицательный → ОХЛАЖДЕНИЕ (5°C)
            ac_target_temp = 45.0 if signal > 0 else 5.0
            _current_temperatures[ac.Y][ac.X] = ac_target_temp
    
    # === УСИЛЕННЫЙ КОЭФФИЦИЕНТ ВЛИЯНИЯ (в 3 раза сильнее) ===
    K_heater_effect = K_signal*3 #* 3.0
    K_ac_effect = K_signal #* 3.0
    ...
    for i in range(c.ROOM_SIZE_Y):
        for j in range(c.ROOM_SIZE_X):
            ...
            # Источники температуры не пересчитываются
            ...
            for ni, nj, k_base in _neighbor_cache[i][j]:
                ...
                if colors[ni][nj] == c.HEATER_COLOR:
                    ...
                    if 0 <= heater_group < len(base_signals):
                        ...
                        power = max(signal, 0.0)  # только нагрев
                        dtemp += K_heater_effect * power * dt_local
                elif colors[ni][nj] == c.AC_COLOR:
                    ac_group = _ac_group_map[ni][nj]
                    if 0 <= ac_group < len(base_signals):
                        signal = base_signals[ac_group]
                        power = abs(signal)  # работает в обоих режимах
                        dtemp += K_ac_effect * power * dt_local
                else:
    ...
    # Защита: принудительно сохраняем фиксированные температуры
    for heater in heaters:
